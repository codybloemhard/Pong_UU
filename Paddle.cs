...
namespace Pong
{
    public class Paddle : GameObject
    {
        ...
        {
            double x = ball.Speed.X;
            double y = ball.Speed.Y;
            double ballAngle;
            ballAngle = Math.Atan2(y, x);
            ballAngle = ballAngle * 57.30;
            return ballAngle;
        }
        public override void Update(GameTime gameTime)
        {
            ...
            if (Collides(ball))
            {
                ...
                double random = Pong.Random.NextDouble();
                if(random >= 0.5)
                {
                    
                }

                ball.Speed = new Vector2(ball.Speed.X * -1, ball.Speed.Y * 1.05f);
